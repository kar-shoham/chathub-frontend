...
import { createGroup, renameGroupName } from '../redux/actions/chatAction';

const UpdateGroup = ({ createChatOpen, toggleCreateChatOpen, chatName, currentChat }) => {
  ...
  let renameGroupChat = async() => {
    if(!groupName){
      return toast.error('Please enter a group name to update!')
    }
    dispatch(renameGroupName({groupId: currentChat, groupName}))
  }
  return (
      ...
      <Box width={'100vw'} height={'100vh'} sx={{ display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
        <Stack borderRadius={'8px'} width={{xs:'80vw', sm:'24vw'}} bgcolor={'white'} sx={{ alignItems: 'center', position: 'relative' }} p={5} gap={4}>
          ...
          <Stack direction={'row'} gap={1} sx={{alignItems: 'center'}}>
            ...
            <Button variant='contained' onClick={() => renameGroupChat()}>UPDATE</Button>
          </Stack>
          <Stack gap={0.5}>
